...
public final class PoetTypeSignatureWriter extends SignatureVisitor {
	...
	private <T extends TypeName> T annotate(T input) {
		return net.fabricmc.filament.mappingpoet.signature.AnnotationAwareDescriptors.annotate(input, storage);
	}
	...
	public void visitClassType(String internalName) {
		Map.Entry<ClassName, TypeAnnotationBank> entry = net.fabricmc.filament.mappingpoet.signature.AnnotationAwareDescriptors.annotateUpTo(internalName, storage, context);
	...
	private void collectLastTypeArgument() {
		if (activeTypeArgument != null) {
			...
			used = net.fabricmc.filament.mappingpoet.signature.AnnotationAwareDescriptors.annotate(used, storage.advance(TypePath.TYPE_ARGUMENT, params.size()));
			params.add(used);
